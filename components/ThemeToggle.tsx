...
import { isDarkModeAtom } from '@/stores/theme'
...
export default function ThemeToggle() {
  ...
  useEffect(() => {
    ...
    // localStorage 직접 업데이트 (jotai가 자동으로 하지만 확실히 하기 위해)
    localStorage.setItem('isDarkMode', isDarkMode.toString())
  }, [isDarkMode, mounted])

  const handleToggle = () => {
    console.log('토글 전:', isDarkMode)
    setIsDarkMode(!isDarkMode)
    console.log('토글 후:', !isDarkMode)
  }

  // 하이드레이션 완료 전에는 렌더링하지 않음
  if (!mounted) {
    return (
      <div className="relative p-2 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 w-10 h-10"></div>
    )
  }
